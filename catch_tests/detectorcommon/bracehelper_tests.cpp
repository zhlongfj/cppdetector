    ...
    GIVEN("") {
        WHEN("") {
            THEN("The function declaration and implementation are on the same line") {
                ...
                auto count = braceHelper.calculateCountOfOpenBrace("bool findLeftBrace() { return m_findBrace; }");
                CHECK(count == 0);
            ...
            THEN("The function implementation contains double quotes") {
                ...
                {
                    count = braceHelper.calculateCountOfOpenBrace(item);
                }
                CHECK(count == 0);
                CHECK(braceHelper.findOpenBrace());
            }
            THEN("There are more close braces than open braces") {
                BraceHelper braceHelper;
                auto count = braceHelper.calculateCountOfOpenBrace("bool findLeftBrace() { return m_findBrace; }");
                CHECK_THROWS(braceHelper.calculateCountOfOpenBrace("return m_findBrace; }") < 0);
                CHECK(braceHelper.findOpenBrace());
            }
        }
    }
